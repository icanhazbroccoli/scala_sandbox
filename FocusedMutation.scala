package io.whitebox.scala.sandbox.focused_mutation
...
object FocusedMutation {
  ...
  def infinitiveTestPurchases: Stream[Purchase] = makeTestPurchase #:: infinitiveTestPurchases

  def immutableSequenceEventProcessing(count: Int) = {
    val testPurchases = infinitiveTestPurchases.take(count)
